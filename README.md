...
>
> 20230514：使用`os.path.normpath()`函数规范化路径，修改后代码在Windows和Ubuntu系统中都能正确运行，**暂停更新**`ubuntu-branch`分支
...
## 20230514-1 添加 检查当前运行配置与已保存配置是否一致

1. 使用Differ类计算当前运行配置与已保存配置差异
2. 写入文件记录差异

## 20230514-2 修改路径拼接 os.path.normpath()

1. 使用`os.path.normpath()`函数来规范化路径
2. 修改后代码在Windows和Ubuntu系统中都能正确运行
3. **暂停更新`ubuntu-branch`分支代码**